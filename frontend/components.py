"""Các khối UI tái dùng -- render_results, sidebar, input theo từng mode.
Tách khỏi app.py để logic hiển thị không lẫn vào control flow chính."""
...
def render_sidebar() -> tuple[str, int, int]:
    """Sidebar chọn loại truy vấn + tham số hiển thị. Trả về (mode, top_n, n_cols)."""
    with st.sidebar:
        st.header("Tuỳ chọn")
...
def render_results(results: list[dict], api: AicApiClient, n_cols: int = 4):
    """Hiển thị lưới card ảnh keyframe, mỗi card có score/video_id/frame_id/timestamp."""
    if not results:
    ...
    for i, r in enumerate(results):
        ...
        with cols[i % n_cols]:
            ...
            st.image(api.keyframe_image_url(r["frame_id"]), use_container_width=True)
            st.markdown(
                f'<div class="frame-meta">'
                f'<span class="score-badge">score {score:.3f}</span><br>'
                f'<b>Video:</b> {r["video_id"]}<br>'
                f'<b>Frame ID:</b> {r["frame_id"]}<br>'
                f'<b>Time:</b> {r["timestamp"]:.1f}s'
                f'</div>',
...
def render_kis_input() -> str | None:
    """Ô input cho KIS -- trả về query khi bấm Tìm, None nếu chưa submit."""
...
def render_qa_input() -> str | None:
    """Ô input cho Q&A -- trả về query khi bấm Hỏi, None nếu chưa submit."""
...
def render_trake_input() -> list[str] | None:
    """Ô input cho TRAKE -- trả về list events khi bấm Tìm, None nếu chưa submit."""
